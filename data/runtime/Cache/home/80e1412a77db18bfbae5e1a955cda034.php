...
<div class="main_wrap">
    <div class="pt25">
    <div class="space_top mb25">
        <div class="space_info clearfix">
            <?php if($user['cover'] != '' AND ACTION_NAME == 'index'): ?><div class="space_cover" style="height:190px;background-image:url(<?php echo attach($user['cover'], 'cover');?>);">
                <div class="iwc_ct"><h1><?php echo ($user["username"]); ?></h1></div>
            </div>
            <?php else: ?>
            <div class="left fl">
                <a href="<?php echo U('space/index', array('uid'=>$user['id']));?>" target="_blank"><img src="<?php echo avatar($user['id'], 100);?>" alt="" class="avatar fl r5"></a>
                <div class="user_profile fl">
                    <span class="uname"><?php echo ($user["username"]); ?></span><br>
                    <div class="home_follow">
                        <?php if($visitor['id'] == $user['id']): ?><div class="see_more_info fl"><a target="_blank" href="<?php echo U('space/info');?>"><?php echo L('user_info');?></a>(<a target="_blank" href="<?php echo U('user/index');?>"><?php echo L('setting');?></a>)</div>
                        <?php else: ?>
                        <div class="J_follow_bar fl" data-uid="<?php echo ($user["id"]); ?>">
                            <?php switch($user["ship"]): case "0": ?><a href="javascript:;" class="J_fo_u fo_u_btn"><?php echo L('follow');?></a><?php break;?>
                            <?php case "1": ?><span class="fo_u_ok"><?php echo L('followed');?></span><a href="javascript:;" class="J_unfo_u green"><?php echo L('cancel');?></a><?php break;?>
                            <?php case "2": ?><span class="fo_u_all"><?php echo L('follow_mutually');?></span><a href="javascript:;" class="J_unfo_u green"><?php echo L('cancel');?></a><?php break; endswitch;?>
                        </div>
                        <div class="see_more_info fl ml10">
                            <a href="<?php echo U('space/info', array('uid'=>$user['id']));?>"><?php echo L('see_user_info');?></a>
                        </div><?php endif; ?>
                    </div>
                </div>
            </div>
            <div class="right fr">
                <div class="collect_list">
                    <a href="<?php echo U('space/follow', array('uid'=>$user['id']));?>" class="ft18"><?php echo ($user["follows"]); ?></a><br>
                    <span><?php echo L('follow');?></span>
                </div>
                <div class="collect_list">
                    <a href="<?php echo U('space/fans', array('uid'=>$user['id']));?>" class="ft18"><?php echo ($user["fans"]); ?></a><br>
                    <span><?php echo L('fans');?></span>
                </div>
                <div class="collect_list bd_none">
                    <a class="ft18"><?php echo ($user["likes"]); ?></a><br>
                    <span><?php echo L('belike');?></span>
                </div>
            </div><?php endif; ?>
        </div>
        <div class="space_nav">
            <?php if(is_array($space_nav_list)): $i = 0; $__LIST__ = $space_nav_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><a <?php if($key == $space_nav_curr): ?>class="current"<?php endif; ?> href="<?php echo ($nav["url"]); ?>"><?php echo ($nav["text"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php if($visitor['id'] == $user['id']): ?><a class="cus_cover fr" href="<?php echo U('user/custom');?>"><?php echo L('custom_cover');?></a><?php endif; ?>
        </div>
    </div>
</div>
    <div class="clearfix">
        <div class="space_cover_main fl">
            <div class="cover_album mb20">
                <h2 class="clearfix">
                    <i><?php if($user['id'] == $visitor['id']): echo L('me'); else: echo ($user["username"]); endif; echo L('ofalbum');?></i> 
                    <a href="<?php echo U('space/album', array('uid'=>$user['id']));?>" title="<?php echo L('show_all_album');?>" class="album_all"><?php echo L('show_all_album');?></a>
                    <?php if($user['id'] == $visitor['id']): ?><a href="javascript:;" class="J_createalbum_btn"><?php echo L('create_album');?></a><?php endif; ?>
                </h2>
                <!--专辑-->
                <ul class="album_list clearfix">
                    <?php if(is_array($album_list)): $i = 0; $__LIST__ = $album_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$album): $mod = ($i % 2 );++$i;?><li class="J_album_item album_item" data-aid="<?php echo ($album["id"]); ?>">
                        <ul>
                            <?php $__FOR_START_1664356623__=0;$__FOR_END_1664356623__=C('pin_album_cover_items');for($i=$__FOR_START_1664356623__;$i < $__FOR_END_1664356623__;$i+=1){ ?><li class="<?php if($i == 0): ?>big<?php elseif($i == 1): ?>left small<?php else: ?>small<?php endif; ?>">
                        ...
                        <h4><a target="_blank" href="<?php echo U('album/detail', array('id'=>$album['id']));?>" title="<?php echo ($album["title"]); ?>"><?php echo ($album["title"]); ?></a></h4>
                        <p class="num"><a target="_blank" href="<?php echo U('album/detail', array('id'=>$album['id']));?>"><?php echo ($album["items"]); echo L('piece_items');?></a></p>
                        <a target="_blank" href="<?php echo U('album/detail', array('id'=>$album['id']));?>" class="album_link"></a>
                        <?php if($user['id'] == $visitor['id']): ?><div class="J_control edit">
                            <a href="javascript:;" class="J_shareitem_btn add" data-aid="<?php echo ($album["id"]); ?>"> + <?php echo L('album_add_item');?></a>
                            <a href="javascript:;" class="J_edit"><?php echo L('edit');?></a>
                            <a href="javascript:;" class="J_del"><?php echo L('delete');?></a>
                        </div><?php endif; ?>
                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            </div>
            <!--分享-->
            <h2 class="user_show_title mb20"><?php if($user['id'] == $visitor['id']): echo L('me'); else: echo ($user["username"]); endif; echo L('ofitem');?></h2>
            <div class="pos_r">
                <div id="J_waterfall" class="wall_container clearfix" data-uri="__ROOT__/?m=space&a=item&uid=<?php echo ($user["id"]); ?>&sort=new">
        ...
        <?php if(!empty($item['comment_list'])): ?><ul class="rep_list">
            <?php $__FOR_START_1799307133__=0;$__FOR_END_1799307133__=C('pin_item_cover_comments');for($i=$__FOR_START_1799307133__;$i < $__FOR_END_1799307133__;$i+=1){ if(!empty($item['comment_list'][$i])): ?><li class="rep_f">
        ...
        <div class="space_cover_side fr">
            <div class="side_piece mb20">
                <div class="side_userinfo">
                    <div class="info">
                        <div class="avatar">
                            <a href="<?php echo U('space/index', array('uid'=>$user['id']));?>" title="<?php echo ($user["username"]); ?>"><img src="<?php echo avatar($user['id'], 200);?>" alt="<?php echo ($user["username"]); ?>"></a>
                        </div>
                        <p class="nickname">
                            <a href="<?php echo U('space/index', array('uid'=>$user['id']));?>" title="<?php echo ($user["username"]); ?>"><?php echo ($user["username"]); ?></a>
                        </p>
                        <div class="record_wrap">
                            <ul class="record clearfix">
                                <li><a href="<?php echo U('space/follow', array('uid'=>$user['id']));?>"><span class="ft14"><?php echo ($user["follows"]); ?></span><span><?php echo L('follow');?></span></a></li>
                                <li><a href="<?php echo U('space/fans', array('uid'=>$user['id']));?>"><span class="ft14"><?php echo ($user["fans"]); ?></span><span><?php echo L('fans');?></span></a></li>
                                <li class="bd_none"><a title="<?php echo L('belike');?>"><span class="ft14"><?php echo ($user["likes"]); ?></span><span><?php echo L('belike');?></span></a></li>
                            </ul>
                        </div>
                        <div class="detail clearfix">
                            <a href="<?php echo U('space/info', array('uid'=>$user['id']));?>"><?php echo L('show_more');?></a>
                            <?php if($user['id'] == $visitor['id']): ?><a href="<?php echo U('user/index');?>"><?php echo L('setting');?></a><?php endif; ?>
                        </div>
                    </div>
                </div>
            </div>
